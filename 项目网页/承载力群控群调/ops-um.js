    ...
    if (umState.inited) return;
    {
      ...
      const load = () => {
        ...
        const sanitize = (payload) => {
          ...
          const roles = (Array.isArray(src.roles) ? src.roles : [])
            ...
            .map((row) => ({
              ...
              perms: uniq(Array.isArray(row.perms) ? row.perms : []),
              status: parseTrim(row.status) === "DISABLED" ? "DISABLED" : "ENABLED",
              remark: parseTrim(row.remark),
              createdAtMs: Number(row.createdAtMs) || Date.now(),
              system: false
        ...
        const demo = {
          ...
          roles: [
            { id: "R-OPS-ADMIN", name: "运营超级管理员", code: "OPS-ADMIN", app: "OPS", perms: ["策略分析", "账号查看"], status: "ENABLED", remark: "运营端最高权限", createdAtMs: nowMs - 90 * 86400000, system: true },
            { id: "R-OPS-OP", name: "运营调度员", code: "OPS-OP", app: "OPS", perms: ["策略分析"], status: "ENABLED", remark: "运营调度与策略分析", createdAtMs: nowMs - 85 * 86400000, system: true },
            { id: "R-GRID-ADMIN", name: "供电局管理员", code: "GRID-ADMIN", app: "GRID", perms: ["拓扑查看", "调度下发", "报表导出", "台区账号查看"], status: "ENABLED", remark: "供电局端管理权限", createdAtMs: nowMs - 80 * 86400000, system: true },
            { id: "R-GRID-OP", name: "台区运维员", code: "GRID-OP", app: "GRID", perms: ["拓扑查看", "报表导出"], status: "ENABLED", remark: "台区日常运维", createdAtMs: nowMs - 78 * 86400000, system: true }
          ],
          accounts: [
        ...
        <div class="um-center">
            ...
            <div class="um-main">
              ...
              <section class="um-panel on" data-panel="org">
                <article class="um-block">
                  ...
                  <div class="um-pager"><div>总共 <strong id="um2OrgCount">0</strong> 条</div><div>50 条/页</div></div>
                </article>
              </section>
              ...
              <section class="um-panel" data-panel="acc">
                <article class="um-block">
                  ...
                  <div class="um-pager"><div>总共 <strong id="um2AccCount">0</strong> 条</div><div>50 条/页</div></div>
                </article>
              </section>

              <section class="um-panel" data-panel="role">
                <article class="um-block">
                  <div class="um-filter-grid with-actions actions-2"><div class="f"><label>关键词</label><input id="um2RoleKw" type="text"></div><div class="f"><label>应用端</label><select id="um2RoleAppF"><option value="ALL">全部</option><option value="OPS">运营端</option><option value="GRID">供电局端</option><option value="BOTH">双端</option></select></div><div class="um-filter-actions"><button class="btn p" id="um2RoleSearch" type="button">查询</button><button class="btn s" id="um2RoleReset" type="button">重置</button><button class="btn s" id="um2RoleAdd" type="button">新增角色</button></div></div>
                  <div class="um-note" id="um2RoleMeta">-</div>
                  <div class="um-table"><table><thead><tr><th>角色</th><th>编码</th><th>应用端</th><th>权限</th><th>账号数</th><th>状态</th><th>操作</th></tr></thead><tbody id="um2RoleBody"></tbody></table></div>
                  <div class="um-pager"><div>总共 <strong id="um2RoleCount">0</strong> 条</div><div>50 条/页</div></div>
                </article>
              </section>
            </div>
          </div>
          ...
          <div class="um-modal" id="um2RoleModal">
            <div class="um-block um-modal-card">
              ...
              <div class="um-form-grid"><div class="f"><label>角色名</label><input id="um2RoleName" type="text"></div><div class="f"><label>编码（系统生成）</label><input id="um2RoleCode" type="text" readonly></div><div class="f"><label>应用端</label><select id="um2RoleApp"><option value="OPS">运营端</option><option value="GRID">供电局端</option><option value="BOTH">双端</option></select></div><div class="f"><label>状态</label><select id="um2RoleStatus"><option value="ENABLED">启用</option><option value="DISABLED">停用</option></select></div><div class="f" style="grid-column:span 2;"><label>备注</label><input id="um2RoleRemark" type="text"></div><div class="f" style="grid-column:span 3;"><label>权限配置</label><div id="um2RolePermBox" class="um-check-wrap"></div></div></div>
      ...
      const rolePermOptions = (app) => {
        if (app === "OPS") return [...ROLE_PERMS.OPS];
        if (app === "GRID") return [...ROLE_PERMS.GRID];
        return uniq([...(ROLE_PERMS.OPS || []), ...(ROLE_PERMS.GRID || [])]);
      };
      ...
      const renderRolePermBox = (app, selected = []) => {
        const box = D("um2RolePermBox");
        if (!box) return;
        const options = rolePermOptions(app);
      ...
      const renderSelects = () => {
        ...
        D("um2AccRole").innerHTML = sortDesc(umState.center.roles).map((r) => `<option value="${esc(r.id)}">${esc(r.name)}（${esc(APP_LABEL[r.app] || r.app)}）</option>`).join("");
      };
      ...
      const renderOrgTable = () => {
        ...
        D("um2OrgBody").innerHTML = rows.length ? rows.map((o) => `<tr><td>${esc(o.name)}</td><td>${esc(o.code)}</td><td>${esc(ORG_LABEL[o.type] || o.type)}</td><td><span class="dce-tag ${o.status === "ENABLED" ? "ok" : "off"}">${o.status === "ENABLED" ? "启用" : "停用"}</span></td><td>${umState.center.accounts.filter((a) => inOrg(a.orgId, o.id)).length}</td><td><button class="btn s um-mini-btn" data-org-act="edit" data-id="${esc(o.id)}" type="button">编辑</button><button class="btn s um-mini-btn" data-org-act="del" data-id="${esc(o.id)}" type="button">删除</button></td></tr>`).join("") : `<tr><td colspan="6" style="text-align:center;color:#567099;">暂无记录</td></tr>`;
        D("um2OrgMeta").textContent = `共 ${umState.center.orgs.length} 个组织，筛选 ${rows.length} 个。`;
        const orgCount = D("um2OrgCount");
        if (orgCount) orgCount.textContent = String(rows.length);
      };

      const renderRoleTable = () => {
        const kw = parseTrim(D("um2RoleKw").value).toLowerCase();
        const af = D("um2RoleAppF").value || "ALL";
        const rows = sortDesc(umState.center.roles).filter((r) => (af === "ALL" || r.app === af) && (!kw || [r.name, r.code, r.remark].join("|").toLowerCase().includes(kw)));
        D("um2RoleBody").innerHTML = rows.length ? rows.map((r) => `<tr><td>${esc(r.name)}</td><td>${esc(r.code)}</td><td>${esc(APP_LABEL[r.app] || r.app)}</td><td>${esc((r.perms || []).join("、") || "-")}</td><td>${umState.center.accounts.filter((a) => a.roleId === r.id).length}</td><td><span class="dce-tag ${r.status === "ENABLED" ? "ok" : "off"}">${r.status === "ENABLED" ? "启用" : "停用"}</span></td><td><button class="btn s um-mini-btn" data-role-act="edit" data-id="${esc(r.id)}" type="button">编辑</button><button class="btn s um-mini-btn" data-role-act="del" data-id="${esc(r.id)}" type="button">删除</button></td></tr>`).join("") : `<tr><td colspan="7" style="text-align:center;color:#567099;">暂无记录</td></tr>`;
        D("um2RoleMeta").textContent = `共 ${umState.center.roles.length} 个角色，筛选 ${rows.length} 个。`;
        const roleCount = D("um2RoleCount");
        if (roleCount) roleCount.textContent = String(rows.length);
      };
      ...
      const renderAccTable = () => {
        ...
        D("um2AccBody").innerHTML = rows.length ? rows.map((a) => {
        ...
        const accCount = D("um2AccCount");
        if (accCount) accCount.textContent = String(rows.length);
      };
      ...
      D("um2OrgSearch").addEventListener("click", renderOrgTable);
      D("um2OrgReset").addEventListener("click", () => { D("um2OrgKw").value = ""; D("um2OrgStatusF").value = "ALL"; renderOrgTable(); });
      D("um2AccSearch").addEventListener("click", renderAccTable);
      D("um2AccReset").addEventListener("click", () => { D("um2AccKw").value = ""; D("um2AccTypeF").value = "ALL"; D("um2AccStatusF").value = "ALL"; renderAccTable(); });
      D("um2RoleSearch").addEventListener("click", renderRoleTable);
      D("um2RoleReset").addEventListener("click", () => { D("um2RoleKw").value = ""; D("um2RoleAppF").value = "ALL"; renderRoleTable(); });
      ...
      D("um2RoleNew").addEventListener("click", () => {
        ...
        renderRolePermBox("OPS", rolePermOptions("OPS"));
        setMsg("um2RoleMsg", "warn", "已生成角色编码。");
      });
      D("um2RoleApp").addEventListener("change", () => {
        const app = D("um2RoleApp").value || "OPS";
        const options = rolePermOptions(app);
        const current = getCheckedPerms().filter((perm) => options.includes(perm));
        renderRolePermBox(app, current.length ? current : options);
      });
      ...
      refresh();
      renderRolePermBox(D("um2RoleApp").value || "OPS", rolePermOptions(D("um2RoleApp").value || "OPS"));
