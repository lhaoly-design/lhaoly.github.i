(() => {
  ...
  const OPERATIONAL_DATA_CLEAR_ONCE_KEY = 'mobile-clear-operational-data-once-v2';
  ...
  const setupDispatchList = () => {
    ...
    const applyVehiclePreset = (vehicleId) => {
      const preset = findVehicle(vehicleId);
      if (!preset) return;
      setFieldValue('driverName', preset.driverName);
      setFieldValue('driverPhone', preset.contact);
    };
    ...
    const populateFormFromRow = (row) => {
      ...
      if (vehicleId) applyVehiclePreset(vehicleId);
      ...
      setFieldValue('startTime', row.dataset.startTime || '');
      setFieldValue('cancelReason', row.dataset.cancelReason || '');
      setFieldValue('driverName', row.dataset.driverName || '');
      setFieldValue('driverPhone', row.dataset.driverPhone || '');
      setFailureRecordVisibility(
        row.dataset.failureRecords || '',
        row.dataset.failureReason || '',
        row.dataset.failureAt || ''
